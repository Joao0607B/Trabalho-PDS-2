...
int quarto::quantidade(std::string tipo)
{
  int count = 0;
...
void quarto::listar(std::string tipo)
{
  for(int i = 0; i < elementos_.size(); i++)
  {
    if(elementos_[i].tipo == tipo)
...
void quarto::Nomear_Elemento(int qtd, std::string tipo)
{
  for(int i = 0; i < qtd; i++)
    {
...
void quarto::remover_sensor(const std::string& nome_sensor)
{
  for(int i = 0; i < elementos_.size(); i++)
  {
