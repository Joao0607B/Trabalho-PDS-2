...
std::map<std::string, Usuario::usuario> Usuario::cadastro;
    ...
    for (auto& a : cadastro){
      if (a.first == login){
        ...
        if (!existeAdmin() && nivel == 1){             
          cout << "O usuário " << login << " agora é o admin." << endl;
          return;
        }
        else{
          cout << "Já existe um adminstrador. Não é possí­vel inserir outro usuário como admin." << endl;
          return;
        }
      }
    }
...
void Usuario::alteranivel (const string& adminlogin, const string& login, const int& novonivel) {        
  ...
  if (adminEncontrado == false){
    cout << "Apenas o admin pode alterar o ní­vel de acesso de outros usuários." << endl;
    return;
  }
  ...
  for (auto& x : cadastro) {
    if (x.first == login) {
      ...
      cout << "Ní­vel de acesso do usuário " << login << " alterado para " << novonivel << "." << endl;
      return;
    }
  }
...
string Usuario::get_senha(const string& login) const {
...
int Usuario::get_nivel(const string& login) const {
    if (cadastro.find(login) != cadastro.end()) {
...
string Usuario::get_pergunta(const string& login) const {
    if (cadastro.find(login) != cadastro.end()) {
...
string Usuario::get_resposta(const string& login) const {
    if (cadastro.find(login) != cadastro.end()) {
